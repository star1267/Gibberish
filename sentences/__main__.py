...
def _get_sentences(
    path: Path, num_sent, words_insent, one_syllwords, two_syllwords, words
):
    """Reads to see if there is a file with sentences already, if goes to sentences builder function"""
    ...
    # if the sentence csv file does not exist it moves on to building the sentences
        ...
        sent = build_sentence(
            num_sent, words_insent, one_syllwords, two_syllwords, words
        )
...
def run(
    ...
    one_syllwords: Annotated[
        int, typer.Option(prompt="Number of one syllable words")
    ] = 7,
    two_syllwords: Annotated[
        int, typer.Option(prompt="Number of two syllable words")
    ] = 1,
) -> None:
